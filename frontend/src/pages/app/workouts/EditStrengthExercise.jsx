...
export default function EditStrengthExercise() {
    ...
    (async () => {
      try {
        ...
        if (ex.sets && ex.sets.length > 0) {
          ...
          setSets(String(ex.sets.length));
          setReps(firstSet.reps ? String(firstSet.reps) : "");
  ...
  const onSave = async () => {
    ...
    try {
      ...
      modal.showSuccess(
        "Exercise Updated",
        "Your strength exercise has been updated successfully."
      );
      setTimeout(() => navigate("/app/workouts"), 1500);
    } catch (err) {
      console.error("Failed to update strength exercise:", err);
      modal.showError(
        "Update Failed",
  ...
  return (
    <DashboardLayout>
      <Header
        title="Workout Management"
        leftSlot={<h6 className="text-primary">Edit Strength Exercise</h6>}
        rightSlot={
        ...
        <div className="card mt-4 space-y-8">
          <div className="flex flex-col gap-2">
            ...
            <p className="p3 text-subtitle">
              Edit your strength exercise details below.
            </p>
          </div>
