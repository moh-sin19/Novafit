...
async function ensureWorkout(userId, workoutIdFromQS) {
  ...
  return await postJson("/api/workouts", {
    userId,
    date: today,
    notes: "Auto-created for cardio",
  });
}

export default function LogCardioExercise() {
  const navigate = useNavigate();
  const location = useLocation();

  const qs = new URLSearchParams(location.search);
  const initialExerciseId = qs.get("exerciseId");
  const exerciseName = qs.get("name") || "Cardio Exercise";
  ...
  const onSave = async () => {
    ...
    try {
      ...
      // 1) create session-exercise WITH exerciseId
      ...
      modal.showSuccess(
        "Exercise Logged",
        "Your cardio exercise has been logged successfully."
      );
  ...
  return (
    <DashboardLayout>
      <Header
        ...
        leftSlot={<h6 className="text-primary">Add Cardio Exercise</h6>}
        rightSlot={
        ...
        <div className="card mt-4 space-y-8">
          <div className="flex flex-col gap-2">
            ...
            <p className="p3 text-subtitle">
              Log your cardio session details below.
            </p>
          </div>
